...
from components.api_client import CHAT_TIMEOUT, post_json
...
if prompt := st.chat_input("ถามทีมข้อมูลของคุณ..."):
    ...
    with st.chat_message("assistant"):
        with st.status(
            ...
            st.write("Data Scientist จะ critique/sanity check หลัง Analyst เสร็จ — อย่าปิดหน้านี้")
            try:
                data = post_json(
                    "/api/v1/chat/",
                    {
                ...
                error_msg = (
                    f"รอคำตอบเกิน {mins} นาที — pipeline 4 agent บน Ollama LAN อาจช้ากว่านั้น "
                    "ลองถามใหม่ด้วย thread เดิมหรือรอแล้ว refresh (backend อาจตอบเสร็จแล้ว)"
                )
                st.error(error_msg)
                st.session_state.messages.append(
