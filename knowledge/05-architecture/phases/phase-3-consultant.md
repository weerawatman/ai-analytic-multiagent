...
2. `$env:PYTHONPATH="."; python -m pytest backend/tests -q` — ต้องผ่านทั้งหมด (เดิม 66 + ใหม่) ✅ **84 passed**
3. **Live consult smoke (ใช้แค่ API key):** ✅ 2026-07-16 — theme `sales`, job `dc35cb0e…` done ~45s, audit `status=ok`, **SECURITY_OK** (ไม่มี QUERY_RESULT/SQL_RESULT/sample_preview/sample_rows), advice บันทึกใน `team_memory/sales.json`
4. Streamlit: `CONSULTANT_ENABLED=false` → panel ซ่อน + chat เดิมไม่เปลี่ยน; เปิด true → consult ครบวง + โผล่ใน Team Memory panel — API status `enabled=true` พร้อม; เปิด UI ตรวจ panel ได้หลัง restart frontend
5. (เมื่อ Ollama กลับมา) chat review + onboarding coach เต็มวง — Fabric ไม่จำเป็น (discovery/team memory อยู่บน disk แล้ว) — รอ Ollama
6. Commit: `feat(consultant): Claude external consultant with data redaction + feedback parity` → push ✅ `1bf1c5d`
