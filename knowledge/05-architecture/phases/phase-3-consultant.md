...
1. `pip install anthropic` + import check (**ทำเป็นอย่างแรก** — Python 3.14 wheel) ✅ (`anthropic==0.116.0`)
2. `$env:PYTHONPATH="."; python -m pytest backend/tests -q` — ต้องผ่านทั้งหมด (เดิม 66 + ใหม่) ✅ **90+ passed** (รวม offline + UI contract)
3. **Live consult smoke (ใช้แค่ API key):** ✅ 2026-07-16 — theme `sales`, audit `status=ok`, **SECURITY_OK**, advice ใน `team_memory/sales.json` + API คืน `consultant_notes`
4. Streamlit: panel ที่ปรึกษา + Team Memory แสดง `consultant_notes` ✅ — `CONSULTANT_ENABLED=false` → `GET /consultant/status` enabled=false (panel ซ่อน); frontend ที่ `http://127.0.0.1:8501`
5. Chat review + onboarding coach เต็มวง ✅ (2026-07-16):
   - Coach: 4 roles ได้ `[ที่ปรึกษา]` notes + glossary proposals
   - Chat job `62e04a5a…` (model `qwen2.5-coder:7b`): **ทุก step done** รวม `consultant_review=done` + message `agent=consultant`
   - หมายเหตุ: `qwen2.5-coder:14b` บนเครื่องนี้ OOM — ใช้ 7b หรือ Ollama LAN ที่ VRAM พอ
   - สคริปต์: `scripts/verify_phase3_remaining.py`
6. Commit: `feat(consultant): ...` → push ✅ `1bf1c5d` + offline hardening `df0a3bd` + remaining fixes (รอบนี้)
